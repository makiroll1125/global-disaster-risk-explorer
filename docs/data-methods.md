...
All views share `state` in `js/app.js`: metric, type, region, country, start/end years, ranking tab, and yearly/monthly timeline resolution. `filterRows` in `js/data.js` applies the common filters. Contextual views explicitly ignore selected dimensions: type ranking retains all types, country ranking and the period map retain country comparisons, and the streamgraph retains all years and types while respecting geography. Selecting a type emphasizes its ranking bar and stream layer without recomputing their contextual distributions.

The detail timeline uses the selected years for both its horizontal domain and values; the overview uses the complete annual domain. Brush movement updates the shared years; a guard suppresses recursive programmatic brush events. From/To controls and All years synchronize through the same render function.

The map has separate `mapMode`, `mapYear`, `playing` and `speed` state. Yearly playback never changes the historical range or other view totals. Animated mode respects all filters, including country, and uses the maximum annual country value over the entire selected playback range for its fixed square-root color domain. Period mode restores the existing comparison map. Gray means no matching records, and hatching means records exist but the impact is unreported. Playback stops at the final year and pauses when the page is hidden. Reduced-motion preferences suppress D3 transitions.
